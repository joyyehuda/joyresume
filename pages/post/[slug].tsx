      ...
      <div className="dark:bg-slate-900 dark:text-white">
        ...
        <article className="container mx-auto px-2 py-10 space-y-5 transition-all">
          <img
            src={
              post.blogImage?.url ||
              "https://media.graphassets.com/cNtqk0JBRSigm3YqBsCA"
            }
            className="w-full shadow-lg h-40 md:h-60 lg:h-80 object-cover grayscale bg-gray-300 transition-all border-2 border-black"
          />
          <div className="border-b-2 border-black dark:border-white">
            <h1 className="text-3xl font-bold lowercase">{post.title}</h1>
            <p className="italic lowercase mb-3">
              by {post.author.name} •{" "}
              {moment(post.createdAt).format("MMM DD, YYYY")}
            </p>
          </div>
          <div
            className="space-y-5 blogContent"
